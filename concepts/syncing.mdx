...
In most cases it is better to be in sync with the remote branch, it will allow to fix conflicts fast and avoid long merging processes in the future.

Turn it off if you don't want remote branch updates to automatically be synced into your cloud workspace (and if workspace sync is resumed then also to your local workspace), for example not to overload internet connection when large files are constantly updated.
