...
<strong>IMPORTANT NOTE:</strong> Diversion treats existing <code>.gitignore</code> files (both root and nested) the same way as <code>.dvignore</code> files.<br />
This means that when a Git repository is transferred into Diversion, its original ignore rules will be preserved.<br />
In some cases, such as transferring the Unreal Engine source repository from Git, you may want to remove the existing <code>.gitignore</code> files
