...
def verify_webhook_signature(webhook_id, webhook_timestamp, sig_header, secret, payload_body):
    ...
    # Extract the base64 part of the secret (after whsec_ prefix)
    if secret.startswith('whsec_'):
        signing_secret = secret[6:]  # Remove 'whsec_' prefix
    else:
        print("Warning: Secret doesn't start with whsec_, using as-is")
        signing_secret = secret
    
    # Decode the base64 signing secret to get the raw key
    try:
        signing_key = base64.b64decode(signing_secret)
    except Exception as e:
        print(f"Failed to decode signing secret: {e}")
        return False
    ...
    computed_hmac = hmac.new(
        signing_key,  # Use the decoded bytes as key
